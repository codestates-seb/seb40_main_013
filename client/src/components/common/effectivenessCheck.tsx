export const nickNameCheck = (name: string) => {
  const nickreg = /^(?=.*[a-z0-9가-힣])[a-z0-9가-힣]{2,8}$/;
...
export const emailCheck = (email: string) => {
  const emailreg = /^[0-9a-zA-Z]([-.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/;
...
export const pwdCheck = (pwd: string) => {
  const pwdreg = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@!%*#?&])[A-Za-z\d$@!%*#?&]{8,20}$/;
...
export const phoneCheck = (phone: string) => {
  const phonereg = /01[016789]-[0-9]{3,4}-[0-9]{4}/;
